# -*- coding: utf-8 -*-
"""
Build outputs/implications.html — "Implications & shocks": the real-world "so what"
cluster on ONE page (policy applications + wars/pandemics/AI), so the talk has a
single implications stop. Earthy, bilingual. Embeds the existing applications and
shocks pages via same-origin, auto-sized iframes (no content drift, no CSS/JS
collisions, continuous scroll).
"""
...
SECTIONS = [
    ("apply", "applications.html", "Practical applications", "Практически приложения",
     "Where the person-year findings meet policy — the three levers and who they touch.",
     "Където изводите в човеко-години срещат политиката — трите лоста и кого засягат."),
    ("shocks", "shocks.html", "Wars, pandemics &amp; AI", "Войни, пандемии и ИИ",
     "The forces outside the slow demographics — the COVID vacancy collapse, the Ukraine supply shock, and AI reshaping demand.",
     "Силите извън бавната демография — сривът на свободните места при COVID, шокът в предлагането от Украйна и ИИ, преоформящ търсенето."),
]
...
iframe.embed{width:100%;height:900px;border:1px solid var(--line);border-radius:14px;background:#fff;display:block}
...
HTML = (f'<!doctype html><html lang="bg" data-lang="bg"><head><meta charset="utf-8">'
        ...
        f'<title>Implications &amp; shocks — longevity &amp; the labour market</title>'
        f'<style>{CSS}{lang_css()}</style></head><body>{lang_toggle()}<div class="wrap">'
        f'<a class="home" href="../index.html">{T("← Overview", "← Обзор")}</a>'
        f'<h1>{T("Implications &amp; shocks", "Последици и сътресения")}</h1>'
        f'<p class="sub">{T("What the numbers mean in the real world — the policy levers, and the wars, pandemics and AI that sit outside the slow demographics.", "Какво значат числата в реалния свят — лостовете на политиката и войните, пандемиите и ИИ, които стоят извън бавната демография.")}</p>'
        f'<div class="jump">{NAV}</div>{SECS}'
        f'<p class="foot"><a href="../index.html">{T("← back to overview", "← обратно към обзора")}</a> · src/report_implications.py</p>'
        f'</div>{JS}</body></html>')

(OUT / "implications.html").write_text(HTML, encoding="utf-8")
print("wrote outputs/implications.html  (applications + shocks)")
